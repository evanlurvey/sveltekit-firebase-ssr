<script lang="ts">
	import FirebaseCtx from '$lib/FirebaseCtx.svelte';
...
<FirebaseCtx user={data.user ?? null} let:currentUser let:getAuth>
	<div class="flex flex-col">
		<header class="flex justify-between items-center py-1 px-2 text-white bg-indigo-700 shadow-lg">
			...
			{#if currentUser}
				<div class="flex gap-1">
					...
					<form on:submit|preventDefault={logOut(getAuth())}>
		...
		{#if currentUser && !currentUser.emailVerified}
			<div class="px-2 m-2 text-sm text-red-900 bg-red-300 rounded border border-red-500 shadow">
				...
				<span class="underline">{currentUser.email}</span> to verify your account. Please check your
